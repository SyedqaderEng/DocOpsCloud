...
const extractSchema = z.object({
  fileId: z.string(),
  pageNumbers: z.array(z.number().min(1)).min(1),
})
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, pageNumbers } = extractSchema.parse(body)
    ...
    const job = await prisma.processingJob.create({
      data: {
        user_id: userId,
        job_type: JobType.PDF_EXTRACT_PAGES,
    ...
    return NextResponse.json({
      jobId: job.id,
      status: 'PENDING',
      message: `PDF extraction job created for ${pageNumbers.length} pages`,
    })
  } catch (error) {
