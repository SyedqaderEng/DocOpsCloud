...
const rotateSchema = z.object({
  ...
  rotation: z.enum([90, 180, 270]).transform(Number),
})
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, pageNumbers, rotation } = rotateSchema.parse(body)
    ...
    await enqueueJob(
      'pdf-processing',
      {
        ...
        operationParams: { pageNumbers, rotation },
    ...
    return NextResponse.json({
      jobId: job.id,
      status: 'PENDING',
      message: `PDF rotation job created for ${pageNumbers.length} pages`,
    })
  } catch (error) {
    ...
    return NextResponse.json(
      { error: 'Failed to create rotation job' },
      { status: 500 }
    )
  }
}
