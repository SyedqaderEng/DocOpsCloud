...
const schema = z.object({ fileId: z.string(), removeStyles: z.boolean().optional() })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, removeStyles } = schema.parse(body)

    const file = await prisma.file.findFirst({ where: { id: fileId, user_id: userId } })
    if (!file) return NextResponse.json({ error: 'File not found' }, { status: 404 })

    if (!file.file_name.match(/\.xlsx?$/i)) {
      return NextResponse.json({ error: 'Only Excel files supported' }, { status: 400 })
    }

    const job = await prisma.processing_job.create({
      data: {
        user_id: userId, operation_type: 'excel_compress', status: 'queued',
        input_file_ids: [fileId], metadata: { removeStyles },
      },
    })

    await queueManager.addExcelJob({
      jobId: job.id, operationType: 'excel_compress', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { removeStyles },
    })

    await logUsage(userId, 'excel_compress', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue compression' }, { status: 500 })
  }
}
