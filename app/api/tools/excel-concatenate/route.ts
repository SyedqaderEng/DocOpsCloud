...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, columns, separator, newColumnName } = schema.parse(body)
    ...
    const job = await prisma.processing_job.create({
      data: {
        user_id: userId, operation_type: 'excel_concatenate', status: 'queued',
        input_file_ids: [fileId], metadata: { columns, separator: separator || ' ', newColumnName },
      },
    })

    await queueManager.addExcelJob({
      jobId: job.id, operationType: 'excel_concatenate', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { columns, separator: separator || ' ', newColumnName },
    })

    await logUsage(userId, 'excel_concatenate', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue concatenation' }, { status: 500 })
  }
}
