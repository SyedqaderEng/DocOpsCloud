...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, sortColumn, order, sheetName } = schema.parse(body)
    ...
    const job = await prisma.processing_job.create({
      data: {
        user_id: userId, operation_type: 'excel_sort_data', status: 'queued',
        input_file_ids: [fileId], metadata: { sortColumn, order: order || 'asc', sheetName },
      },
    })

    await queueManager.addExcelJob({
      jobId: job.id, operationType: 'excel_sort_data', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { sortColumn, order: order || 'asc', sheetName },
    })

    await logUsage(userId, 'excel_sort_data', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue sort' }, { status: 500 })
  }
}
