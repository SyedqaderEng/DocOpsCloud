...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, columns, sheet } = schema.parse(body)

    const file = await prisma.file.findFirst({ where: { id: fileId, user_id: userId } })
    if (!file) return NextResponse.json({ error: 'File not found' }, { status: 404 })
    if (!file.file_name.match(/\.(xlsx?|csv)$/i)) return NextResponse.json({ error: 'Excel or CSV files only' }, { status: 400 })

    const job = await prisma.processing_job.create({
      data: { user_id: userId, operation_type: 'excel_statistics', status: 'queued', input_file_ids: [fileId], metadata: { columns, sheet } },
    })

    await queueManager.addExcelJob({
      jobId: job.id, operationType: 'excel_statistics', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { columns, sheet },
    })

    await logUsage(userId, 'excel_statistics', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue statistics calculation' }, { status: 500 })
  }
}
