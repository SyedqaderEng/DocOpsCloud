...
const excelToCsvSchema = z.object({
  fileId: z.string(),
  sheetName: z.string().optional(), // specific sheet or all
  delimiter: z.string().optional(), // default comma
})
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, sheetName, delimiter } = excelToCsvSchema.parse(body)
    ...
    // Validate file type
    if (!ALLOWED_MIMETYPES.includes(file.mime_type || '')) {
      return NextResponse.json({
    ...
    await logUsage(userId, 'excel_to_csv', 0)
    ...
    return NextResponse.json({
      ...
      message: 'Excel to CSV conversion job queued',
    })
  } catch (error) {
    ...
    return NextResponse.json({ error: 'Failed to queue Excel to CSV conversion' }, { status: 500 })
  }
}
