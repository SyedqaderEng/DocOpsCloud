...
export async function POST(request: NextRequest) {
  try {
    ...
    const job = await prisma.processing_job.create({
      data: {
        user_id: userId, operation_type: 'excel_transpose', status: 'queued',
        input_file_ids: [fileId], metadata: { sheetName },
      },
    })

    await queueManager.addExcelJob({
      jobId: job.id, operationType: 'excel_transpose', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { sheetName },
    })

    await logUsage(userId, 'excel_transpose', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue transpose' }, { status: 500 })
  }
}
