...
const schema = z.object({ fileId: z.string(), sigma: z.number().min(0.3).max(100).optional() })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, sigma } = schema.parse(body)
    ...
    const job = await prisma.processing_job.create({
      data: { user_id: userId, operation_type: 'image_blur', status: 'queued', input_file_ids: [fileId], metadata: { sigma: sigma || 5 } },
    })

    await queueManager.addImageJob({
      jobId: job.id, operationType: 'image_blur', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { sigma: sigma || 5 },
    })

    await logUsage(userId, 'image_blur', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue blur' }, { status: 500 })
  }
}
