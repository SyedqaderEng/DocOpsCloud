...
const schema = z.object({ fileId: z.string(), brightness: z.number().min(0).max(3) }) // 1 = no change
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, brightness } = schema.parse(body)
    ...
    const job = await prisma.processing_job.create({
      data: { user_id: userId, operation_type: 'image_brightness', status: 'queued', input_file_ids: [fileId], metadata: { brightness } },
    })

    await queueManager.addImageJob({
      jobId: job.id, operationType: 'image_brightness', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { brightness },
    })

    await logUsage(userId, 'image_brightness', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue brightness adjustment' }, { status: 500 })
  }
}
