...
const schema = z.object({ fileId: z.string(), quality: z.number().min(1).max(100).optional() })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, quality } = schema.parse(body)
    ...
    const job = await prisma.processing_job.create({
      data: { user_id: userId, operation_type: 'image_compress', status: 'queued', input_file_ids: [fileId], metadata: { quality: quality || 80 } },
    })

    await queueManager.addImageJob({
      jobId: job.id, operationType: 'image_compress', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { quality: quality || 80 },
    })

    await logUsage(userId, 'image_compress', 0)
