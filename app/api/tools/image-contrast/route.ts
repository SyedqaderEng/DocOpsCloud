...
const schema = z.object({ fileId: z.string(), contrast: z.number().min(0).max(3).optional() })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, contrast } = schema.parse(body)
    ...
    const job = await prisma.processing_job.create({
      data: { user_id: userId, operation_type: 'image_contrast', status: 'queued', input_file_ids: [fileId], metadata: { contrast: contrast || 1.5 } },
    })

    await queueManager.addImageJob({
      jobId: job.id, operationType: 'image_contrast', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { contrast: contrast || 1.5 },
    })

    await logUsage(userId, 'image_contrast', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue contrast adjustment' }, { status: 500 })
  }
}
