...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, format, quality } = schema.parse(body)
    ...
    const job = await prisma.processing_job.create({
      data: { user_id: userId, operation_type: 'image_convert', status: 'queued', input_file_ids: [fileId], metadata: { format, quality } },
    })

    await queueManager.addImageJob({
      jobId: job.id, operationType: 'image_convert', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { format, quality },
    })

    await logUsage(userId, 'image_convert', 0)
