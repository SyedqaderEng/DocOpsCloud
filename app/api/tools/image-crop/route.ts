...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, left, top, width, height } = schema.parse(body)
    ...
    if (!file.file_name.match(IMAGE_EXTENSIONS)) {
      return NextResponse.json({
        error: 'Invalid file type. Supported: JPG, PNG, GIF, WebP, AVIF, TIFF, SVG'
      }, { status: 400 })
    }

    const job = await prisma.processing_job.create({
      data: {
        user_id: userId, operation_type: 'image_crop', status: 'queued',
        input_file_ids: [fileId], metadata: { left, top, width, height },
      },
    })

    await queueManager.addImageJob({
      jobId: job.id, operationType: 'image_crop', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { left, top, width, height },
    })

    await logUsage(userId, 'image_crop', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue crop' }, { status: 500 })
  }
}
