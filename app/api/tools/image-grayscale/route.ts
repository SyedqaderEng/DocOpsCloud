...
export async function POST(request: NextRequest) {
  try {
    ...
    const job = await prisma.processing_job.create({
      data: { user_id: userId, operation_type: 'image_grayscale', status: 'queued', input_file_ids: [fileId], metadata: {} },
    })

    await queueManager.addImageJob({
      jobId: job.id, operationType: 'image_grayscale', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: {},
    })

    await logUsage(userId, 'image_grayscale', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue grayscale conversion' }, { status: 500 })
  }
}
