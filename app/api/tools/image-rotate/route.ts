...
const schema = z.object({
  fileId: z.string(),
  angle: z.number(), // 90, 180, 270 or any angle
  background: z.string().optional(), // hex color for background when rotating odd angles
})
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, angle, background } = schema.parse(body)

    const file = await prisma.file.findFirst({ where: { id: fileId, user_id: userId } })
    if (!file) return NextResponse.json({ error: 'File not found' }, { status: 404 })

    if (!file.file_name.match(IMAGE_EXTENSIONS)) {
      return NextResponse.json({ error: 'Invalid file type. Images only.' }, { status: 400 })
    }

    const job = await prisma.processing_job.create({
      data: {
        user_id: userId, operation_type: 'image_rotate', status: 'queued',
        input_file_ids: [fileId], metadata: { angle, background },
      },
    })

    await queueManager.addImageJob({
      jobId: job.id, operationType: 'image_rotate', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { angle, background },
    })

    await logUsage(userId, 'image_rotate', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue rotation' }, { status: 500 })
  }
}
