...
const schema = z.object({ fileId: z.string(), intensity: z.number().min(0).max(1).optional() })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, intensity } = schema.parse(body)
    ...
    const job = await prisma.processing_job.create({
      data: { user_id: userId, operation_type: 'image_sepia', status: 'queued', input_file_ids: [fileId], metadata: { intensity: intensity ?? 1 } },
    })

    await queueManager.addImageJob({
      jobId: job.id, operationType: 'image_sepia', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { intensity: intensity ?? 1 },
    })

    await logUsage(userId, 'image_sepia', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue sepia effect' }, { status: 500 })
  }
}
