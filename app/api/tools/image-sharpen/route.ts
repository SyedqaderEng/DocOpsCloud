...
const schema = z.object({ fileId: z.string(), sigma: z.number().optional(), flat: z.number().optional(), jagged: z.number().optional() })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, sigma, flat, jagged } = schema.parse(body)
    ...
    const job = await prisma.processing_job.create({
      data: { user_id: userId, operation_type: 'image_sharpen', status: 'queued', input_file_ids: [fileId], metadata: { sigma, flat, jagged } },
    })

    await queueManager.addImageJob({
      jobId: job.id, operationType: 'image_sharpen', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { sigma, flat, jagged },
    })

    await logUsage(userId, 'image_sharpen', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue sharpen' }, { status: 500 })
  }
}
