...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, width, height } = schema.parse(body)
    ...
    const job = await prisma.processing_job.create({
      data: {
        user_id: userId, operation_type: 'image_thumbnail', status: 'queued', input_file_ids: [fileId],
        metadata: { width: width || 150, height: height || 150 },
      },
    })

    await queueManager.addImageJob({
      jobId: job.id, operationType: 'image_thumbnail', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { width: width || 150, height: height || 150 },
    })

    await logUsage(userId, 'image_thumbnail', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue thumbnail creation' }, { status: 500 })
  }
}
