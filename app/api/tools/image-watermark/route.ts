...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, text, position, opacity, fontSize } = schema.parse(body)
    ...
    const job = await prisma.processing_job.create({
      data: {
        user_id: userId, operation_type: 'image_watermark', status: 'queued', input_file_ids: [fileId],
        metadata: { text, position: position || 'center', opacity: opacity || 0.5, fontSize: fontSize || 24 },
      },
    })

    await queueManager.addImageJob({
      jobId: job.id, operationType: 'image_watermark', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { text, position: position || 'center', opacity: opacity || 0.5, fontSize: fontSize || 24 },
    })

    await logUsage(userId, 'image_watermark', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue watermark' }, { status: 500 })
  }
}
