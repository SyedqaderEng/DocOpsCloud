...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, color, imageFileId, opacity } = schema.parse(body)

    if (!color && !imageFileId) return NextResponse.json({ error: 'Color or background image required' }, { status: 400 })
    ...
    const job = await prisma.processing_job.create({
      data: { user_id: userId, operation_type: 'pdf_background', status: 'queued', input_file_ids: [fileId], metadata: { color, imageFileId, opacity: opacity ?? 1 } },
    })

    await queueManager.addPdfJob({
      jobId: job.id, operationType: 'pdf_background', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: { color, imageFileId, opacity: opacity ?? 1 },
    })

    await logUsage(userId, 'pdf_background', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue background addition' }, { status: 500 })
  }
}
