...
export async function POST(request: NextRequest) {
  try {
    ...
    const job = await prisma.processing_job.create({
      data: { user_id: userId, operation_type: 'pdf_grayscale', status: 'queued', input_file_ids: [fileId], metadata: {} },
    })

    await queueManager.addPdfJob({
      jobId: job.id, operationType: 'pdf_grayscale', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: {},
    })

    await logUsage(userId, 'pdf_grayscale', 0)
