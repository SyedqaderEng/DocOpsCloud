...
const linearizeSchema = z.object({
  fileId: z.string(),
})
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId } = linearizeSchema.parse(body)
    ...
    await queueManager.addPdfJob({
      jobId: job.id,
      operationType: 'pdf_linearize',
      userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: {},
    })

    await logUsage(userId, 'pdf_linearize', 0)
    ...
    return NextResponse.json({
      ...
      message: 'PDF linearization job queued. This optimizes the PDF for fast web viewing.',
    })
  } catch (error) {
    ...
    return NextResponse.json({ error: 'Failed to queue PDF linearization' }, { status: 500 })
  }
}
