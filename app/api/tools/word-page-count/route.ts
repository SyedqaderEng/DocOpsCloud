...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!file.file_name.match(/\.docx?$/i)) return NextResponse.json({ error: 'Word documents only (.doc, .docx)' }, { status: 400 })

    const job = await prisma.processing_job.create({
      data: { user_id: userId, operation_type: 'word_page_count', status: 'queued', input_file_ids: [fileId], metadata: {} },
    })

    await queueManager.addWordJob({
      jobId: job.id, operationType: 'word_page_count', userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: {},
    })

    await logUsage(userId, 'word_page_count', 0)
    ...
    return NextResponse.json({ error: 'Failed to queue page count' }, { status: 500 })
  }
}
