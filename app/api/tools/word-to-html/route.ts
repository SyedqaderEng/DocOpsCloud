...
const wordToHtmlSchema = z.object({
  fileId: z.string(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId, includeStyles } = wordToHtmlSchema.parse(body)
    ...
    await logUsage(userId, 'word_to_html', 0)
    ...
    return NextResponse.json({
      ...
      message: 'Word to HTML conversion job queued',
    })
  } catch (error) {
    ...
    return NextResponse.json({ error: 'Failed to queue Word to HTML conversion' }, { status: 500 })
  }
}
