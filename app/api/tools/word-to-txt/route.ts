...
const wordToTxtSchema = z.object({
  fileId: z.string(),
})
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { fileId } = wordToTxtSchema.parse(body)
    ...
    await queueManager.addWordJob({
      jobId: job.id,
      operationType: 'word_to_txt',
      userId,
      files: [{ id: file.id, s3_key: file.s3_key, file_name: file.file_name }],
      options: {},
    })

    await logUsage(userId, 'word_to_txt', 0)
    ...
    return NextResponse.json({
      ...
      message: 'Word to Text conversion job queued',
    })
  } catch (error) {
    ...
    return NextResponse.json({ error: 'Failed to queue Word to Text conversion' }, { status: 500 })
  }
}
